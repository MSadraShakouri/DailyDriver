    ...
    with get_connection_cm() as conn:
        ...
        while True:
            ...
            for row in items:
                ...
                current_ui.print_line(
                    f"  {row['item']}: every {row['desired_interval_days']} day(s) | early: {early_str} | due today: {due_str}"
                )
            ...
            if choice == "q":
                ...
                cur.execute(
                    "SELECT id, desired_interval_days, early_warning_enabled, show_due_today FROM hygiene_config WHERE item=?",
                    (item_name,),
                )
                row = cur.fetchone()
                if not row:
                ...
                ew_str = (
                    current_ui.prompt(
                        f"Early warning? [currently {'on' if current_ew else 'off'}] (Enter=keep, n=toggle): "
                    )
                    .strip()
                    .lower()
                )
                ...
                dt_str = (
                    current_ui.prompt(
                        f"Show due today? [currently {'on' if current_dt else 'off'}] (Enter=keep, n=toggle): "
                    )
                    .strip()
                    .lower()
                )
