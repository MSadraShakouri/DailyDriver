...
from . import clock, entries, schedule
from .history import get_daily_total
...
def get_targets_header_lines(conn):
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT id, kind, name, target_total, logged_total, interval_type,
               interval_value, target_per_interval, paused_until
        FROM target_entries
        ORDER BY kind, name
    """)
    ...
    for entry in entries:
        ...
        next_due = schedule.compute_next_due(entry, today)
