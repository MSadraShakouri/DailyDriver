...
class TestWeather(unittest.TestCase):
    def setUp(self):
        ...
        self.conn.execute('CREATE TABLE weather_log (id INTEGER PRIMARY KEY, temp_c INTEGER, condition_fa TEXT, timestamp INTEGER)')
    ...
    def test_live_today(self, mock_get_weather):
        mock_get_weather.return_value = {
            'temp_c': 30, 'condition_en': 'clear', 'condition_emoji': '☀️', 'timestamp': time.time()
        }
    ...
    def test_cached_past(self):
        ...
        self.conn.execute("INSERT INTO weather_log (temp_c, condition_fa, timestamp) VALUES (?,?,?)",
                          (25, 'صاف', ts))
