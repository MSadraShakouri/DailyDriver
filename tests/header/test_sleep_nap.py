...
from dailydriver.display.header.sleep import get_sleep_str, get_nap_str
...
class TestSleep(unittest.TestCase):
    def setUp(self):
        ...
        self.conn.execute('CREATE TABLE sleep_logs (id INTEGER PRIMARY KEY, jalali_date TEXT, sleep_time INTEGER, wake_time INTEGER, duration_minutes INTEGER)')
...
class TestNap(unittest.TestCase):
    def setUp(self):
        self.conn = sqlite3.connect(':memory:')
        self.conn.row_factory = sqlite3.Row
        self.conn.execute('CREATE TABLE nap_logs (id INTEGER PRIMARY KEY, jalali_date TEXT, start_time INTEGER, duration_minutes INTEGER)')
        self.today = '1405-02-21'
