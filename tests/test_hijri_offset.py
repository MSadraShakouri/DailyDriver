...
import dailydriver.features.calendar.catalog as ce_module
from dailydriver.features.calendar.hijri import get_hijri_offset, set_hijri_offset
...
class TestHijriOffset(unittest.TestCase):
    ...
    def test_get_offset_reads_file(self):
        with patch(
            "dailydriver.features.calendar.hijri.open",
    ...
    def test_get_offset_file_missing(self):
        with patch("dailydriver.features.calendar.hijri.open", side_effect=FileNotFoundError):
    ...
    def test_set_offset_writes_file(self):
        ...
        with (
            patch("dailydriver.features.calendar.hijri.open", m, create=True),
            patch("dailydriver.features.calendar.hijri.jdatetime.date") as mock_jd,
        ):
    ...
    def test_set_offset_invalidates_cache(self):
        ...
        with (
            patch("dailydriver.features.calendar.hijri.open", mock_open(), create=True),
            patch("dailydriver.features.calendar.hijri.jdatetime.date") as mock_jd,
        ):
            fake_today = MagicMock()
            fake_today.strftime.return_value = "1405-02-30"
