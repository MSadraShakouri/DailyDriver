...
from dailydriver.features.qada import commands, logging
...
class TestQadaFastingCommands(unittest.TestCase):
    def setUp(self):
        ...
        self.patchers = [
            ...
            for module in ("commands", "logging", "schedule", "entries")
        ]
    ...
    def test_log_fasting_inserts_log(self):
        ...
        result = logging.log_fasting(eid)
    ...
    def test_log_fasting_uses_specified_now(self):
        ...
        eid = self._add_fasting_entry(created_at=today_start)
        fake_now = 1234567890
        logging.log_fasting(eid, now=fake_now)
        row = self.conn.execute(
    ...
    def test_qada_fasting_yes_parses(self):
        with patch("dailydriver.features.qada.commands.log_fasting") as mock_log:
            mock_log.return_value = "Logged"
            result = commands._parse_fasting("yes")
