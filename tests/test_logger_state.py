...
class TestLoggerState(unittest.TestCase):
    ...
    def test_get_last_action_time_value(self):
        self.conn.execute(
            "INSERT INTO meta (key, value) VALUES ('last_action', '123456')"
        )
        with patch(
            "dailydriver.core.logger.get_connection_cm", return_value=self.mock_cm
        ):
    ...
    def test_discard_pending_start_removes(self):
        self.conn.execute(
            "INSERT INTO meta (key, value) VALUES ('pending_start', '999')"
        )
        with patch(
            "dailydriver.core.logger.get_connection_cm", return_value=self.mock_cm
        ):
    ...
    def test_clear_pending_start(self):
        self.conn.execute(
            "INSERT INTO meta (key, value) VALUES ('pending_start', '111')"
        )
        with patch(
            "dailydriver.core.logger.get_connection_cm", return_value=self.mock_cm
        ):
    ...
    def test_start_duplicate_great_event_raises(self):
        self.conn.execute(
            "INSERT INTO meta (key, value) VALUES ('great_event_start', '1')"
        )
        self.conn.execute(
            "INSERT INTO meta (key, value) VALUES ('great_event_categories', 'test')"
        )
        with patch(
            "dailydriver.core.logger.get_connection_cm", return_value=self.mock_cm
        ):
    ...
    def test_clear_great_event(self):
        self.conn.execute(
            "INSERT INTO meta (key, value) VALUES ('great_event_start', '2')"
        )
        self.conn.execute(
            "INSERT INTO meta (key, value) VALUES ('great_event_categories', 'x')"
        )
        with patch(
            "dailydriver.core.logger.get_connection_cm", return_value=self.mock_cm
        ):
