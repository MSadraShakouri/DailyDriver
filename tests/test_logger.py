...
class TestLogFreeTextTimeConversion(unittest.TestCase):
    def setUp(self):
        ...
        self.patch_categories = patch('dailydriver.core.logger.find_matching_categories', return_value=[('programming', 1)])
    ...
    def test_l6m_passes_timestamp(self):
        ...
        interpretation = TimeInterpretation(
            start=start, end=end,
            duration_minutes=6,
            label="last 6m (14:24 → 14:30)", priority=1
        )
        with patch('dailydriver.core.logger.parse_time_expressions', return_value=[interpretation]):
    ...
    def test_explicit_time_interpretation_selected(self):
        ...
        interp1 = TimeInterpretation(
            start=datetime(2026, 5, 20, 9, 18, 0), end=datetime(2026, 5, 20, 9, 24, 0),
            duration_minutes=6, label="09:18 → 09:24", priority=1
        )
        interp2 = TimeInterpretation(
            start=datetime(2026, 5, 20, 21, 18, 0), end=datetime(2026, 5, 20, 21, 24, 0),
            duration_minutes=6, label="21:18 → 21:24", priority=2
        )
        with patch('dailydriver.core.logger.parse_time_expressions', return_value=[interp1, interp2]):
